...
const clearListDisplay = () => {
  ...
  deleteContents(parentElement); //helper function
};
...
const renderList = () => {
  const list = toDoList.getList();
  list.forEach((item) => {
    buildListItem(item); // helper function only one task of it to build the list
  });
};
...
const buildListItem = (item) => {
  ...
  check.id = item.getId();
  check.tabIndex = 0;
  addclickListenerToCheckbox(check); // helper function
